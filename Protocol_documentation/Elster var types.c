...
bool SetElsterTabIndex()
{
  for (int i = 0; i <= (int) High(ElsterTabIndex); i++)
    ElsterTabIndex[i] = -1;

  for (int i = 0; i <= (int) High(ElsterTable); i++)
    if (ElsterTabIndex[ElsterTable[i].Index] == -1)
      ElsterTabIndex[ElsterTable[i].Index] = i;
 
  return true;
}

static bool ElsterTabIndexOk = SetElsterTabIndex();
...
int GetElsterTableIndex(unsigned short Index)
{
  return ElsterTabIndex[Index];
}

const char * GetElsterTableName(unsigned short Index)
{
